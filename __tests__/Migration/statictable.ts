import { convert, isOldStructure } from "../../src/modules/Migration/statictable.ts";
import * as oldStructure from "./__mocks__/widgetsOldStructure.json";

const oldWidget = oldStructure.statictableStructure;

describe("Static table widget - migration suite", () => {
  /**
  ...
  it("converts base structure properly", () => {
    ...
    expect(newStructure.type).toEqual("static_table");
  });
  ...
  it("correctly migrate first row to bold and auto size", () => {
    ...
    for (const variable of newStructure1.display.variables) {
      if (variable?.data_type === "text") {
        expect(variable?.show_bold).toBeTruthy();
      }

      expect(variable.size).toBe("auto");
    }
  });
});
