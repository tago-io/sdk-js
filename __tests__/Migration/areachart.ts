import { convert, isOldStructure } from "../../src/modules/Migration/areachart.ts";
import * as oldStructure from "./__mocks__/widgetsOldStructure.json";

const oldWidget = oldStructure.areachartStructure;

describe("area chart widget - migration suite", () => {
  /**
  ...
  it("converts base structure properly", () => {
    ...
    expect(newStructure.type).toEqual("area_chart");
    expect(newStructure.display.show_point_values).toBeFalsy();
  });
  ...
  /**
   * Check if correctly applies line smooth and max points
   */
  it("correctly insert line curve and max points", () => {
    const newDisplay = {
      ...oldWidget.display,
    };
