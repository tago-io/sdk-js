import { convert, isOldStructure } from "../../src/modules/Migration/custom.ts";
import * as oldStructure from "./__mocks__/widgetsOldStructure.json";

const oldWidget = oldStructure.customStructure;

describe("Custom widget - migration suite", () => {
  /**
  ...
  it("converts base structure properly", () => {
    ...
    expect(newStructure.type).toEqual("iframe");
  });
  ...
  it("should not convert data, realtime, or analysis", () => {
    ...
    expect(newStructure.analysis_run).toEqual(oldWidget?.analysis_run);
  });
