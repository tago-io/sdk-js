...
class Profile extends TagoIOModule<GenericModuleParams> {
  ...
  /**
   * Renew and retrieves a new token for 6 hours
   * @param profileID Profile ID
   * @param token Token params for new token
   */
  public async renewProfileToken(profileID: GenericID, token: GenericToken): Promise<TokenCreateResponse> {
    let result = await this.doRequest<TokenCreateResponse>({
      path: `/profile/${profileID}/token/renew`,
      method: "POST",
      body: {
        name: "Generated by TagoIO Renew",
