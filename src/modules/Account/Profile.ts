...
class Profile extends TagoIOModule<GenericModuleParams> {
  ...
  /**
   * Logout of all sessions that are logged into the profile
   * @param profileID Profile ID
   */
  public async logoutFromAllSessions(profileID: GenericID): Promise<string> {
    const result = await this.doRequest<string>({
      path: `/profile/${profileID}/token/autogenerated/all`,
      method: "DELETE",
    });
